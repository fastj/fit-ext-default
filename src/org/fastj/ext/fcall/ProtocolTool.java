...
public class ProtocolTool {
	...
	public static Proxy getProxy(ParameterTable table) throws ParamIncertitudeException, DataInvalidException
	{
		if (table.lcontains("proxy") || (table.getParent() != null &&  table.getParent().lcontains("proxy")))
		{
			String pstr = StringUtil.expendVar("proxy", table);
			String pargs[] = StringUtil.readFuncParam(pstr);
			if (pargs.length == 3 || pargs.length == 5)
			{
				Proxy proxy = new Proxy();
				proxy.setProxyIp(pargs[0]);
				proxy.setProxyPort(Integer.valueOf(pargs[1]));
				proxy.setType(pargs[2]);
				proxy.setName(pargs.length > 3 ? pargs[3] : null);
				proxy.setPassword(pargs.length > 4 ? pargs[4] : null);
				return proxy;
			}
		}
